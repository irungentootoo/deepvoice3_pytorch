...
def build_from_path(in_dir, out_dir, num_workers=1, tqdm=lambda x: x):
    ...
    for speaker_id, json_path in enumerate(json_paths):
        ...
        if json_path.endswith("json"):
            ...
            raise Exception(f" [!] Unknown metadata format: {json_path}")
        ...
        for path in info.keys():
            if not os.path.exists(path):
                ...
                if not os.path.exists(new_path):
                    print(f" [!] Audio not found: {[path, new_path]}")
                    continue
            else:
                new_path = path
        ...
        for audio_path, text in info.items():
            if isinstance(text, list):
                text = text[-1] if hparams.ignore_recognition_level == 0 else text[0]
            ...
            queue_count += 1
        print(f" [*] Appended {queue_count} entries in the queue")
    ...
    print(
        f" [!] Skip recognition level: {hparams.ignore_recognition_level} ({ignore_description[hparams.ignore_recognition_level]})"
    )

    if num_speakers == 1:
        print(" [!] Single-speaker mode activated!")
    else:
        print(f" [!] Multi-speaker({num_speakers}) mode activated!")
...
def _process_utterance(out_dir, text, wav_path, speaker_id=None):
    ...
    elif hparams.process_only_htk_aligned:
        return None
    else:
    ...
    # e.g. Recording0.wav
    spectrogram_filename = f'spec-{speaker_id}-{wav_name}.npy'
    mel_filename = f'mel-{speaker_id}-{wav_name}.npy'
...
def _process_utterance_single(out_dir, text, wav_path):
    ...
    wav_name = os.path.basename(wav_path)
    wav_name = os.path.splitext(wav_name)[0]
    spectrogram_filename = f'spec-{wav_name}.npy'
    mel_filename = f'mel-{wav_name}.npy'
    np.save(os.path.join(out_dir, spectrogram_filename), spectrogram.T, allow_pickle=False)
    np.save(os.path.join(out_dir, mel_filename), mel_spectrogram.T, allow_pickle=False)
